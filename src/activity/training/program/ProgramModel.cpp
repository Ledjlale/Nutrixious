	...
	: QObject{parent}
{
	...
	connect(this, &ProgramModel::targetProgramModelChanged, [this](){
		mExercises.clear();
		if(mTargetProgramModel){
			for(auto model : mTargetProgramModel->getExercises()){
				auto newExercise = model->cloneTraining();
				connect(newExercise, &ExerciseModel::finished, this, &ProgramModel::nextExercise);
...
/*
	...
	QSqlQuery query;
	if(mDbId == 0){
		query.prepare("INSERT INTO programs(name,description) VALUES(?,?) RETURNING id");
		...
		for(auto exercise : mExercises){
			exercise->saveProgram(mDbId);
		}
	}else{
		query.prepare("UPDATE programs SET name=?, description=? WHERE id=?");
		query.addBindValue(mName);
		query.addBindValue(mDescription);
		query.addBindValue(mDbId);
		qDebug() << "Update program : " << mDbId << query.exec();
		for(auto exercise : mExercises){
			exercise->saveProgram(mDbId);
		}
	}
...
//----------------------		Strength
	...
	while (query.next()) {
		...
		dynamic_cast<StrengthModel*>(*currentExercise)->addSet(model);
	}
...
QList<ExerciseModel*>& ProgramModel::getExercises(){
	return mExercises;
}
...
void ProgramModel::addExercise(ExerciseModel *model, bool keepId) {
	...
	if(programOrder < 0) {
		mExercises.push_back(model->clone());
		...
		if( mExercises.size() == 0){
			...
			while(it != mExercises.end() && (*it)->getProgramOrder() <= programOrder)
				++it;
			insertedModel = *mExercises.insert(it, model->clone());
		}
	}
	if(keepId)
